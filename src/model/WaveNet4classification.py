...
class WaveNet4classification(nn.Module):
    def __init__(self, batch_size, n_classes=41, n_layers=2, n_blocks=6, n_hidden=32, embedding_dim=None, dilation_channels=32, residual_channels=32, bias=True, gc_cardinality=None, lc_cardinality=None, skip_channels=32, use_BN=True):
        super(WaveNet4classification, self).__init__()
        ...
        self.causal_conv_input = nn.Conv1d(in_channels=dilation_channels,
                                                 ...
                                                 kernel_size=1,    #kernel size of the first causal layer
                                                 stride=1,
                                                 # padding=4,
                                                 bias=bias)


        self.dilations_each_block = [4**i for i in range(n_blocks)]
        ...
        self.receptive_field = self.dilations_each_block[-1]*4 * n_layers - (n_layers - 1)
    ...
    def forward(self, input, gc=None, lc=None, is_training=None):
        ...
        for i, dilation_layer in enumerate(self.dilation_layers[1:]):
            ...
            skip_out = torch.add(skip_out[:, :, dilation_layer.dilation*(dilation_layer.kernel_size-1):], current_skip_out)

            # skips.append(skip_out)

        # x = reduce((lambda a, b : torch.add(a, b)), skips)
        x = self.ReLU(residual_out)
...
class Residual_Block(nn.Module):
    def __init__(self, batch_size, dilation, residual_channels=32, dilation_channels=32, kernel_size=4, bias=False, gc_cardinality=None, gc_channels=32, lc_cardinality=None, lc_channels=32, skip_channels=32, use_BN=True):
    ...
    def forward(self, input, gc=None, lc=None):
        ...
        # print("current dilation = {}".format(self.dilation))
        # print(input.shape)
        ...
        residual_out = self.residual_conv(x) + input[:, :, self.dilation*(self.kernel_size-1):]
